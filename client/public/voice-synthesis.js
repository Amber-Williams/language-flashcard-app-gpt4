...
const voiceSelect = document.querySelector("#voice-dropdown");
...
function getLanguageCode(language) {
  return languageCodes.find((lang) => lang.name.includes(language))?.code;
}
...
function populateVoiceList() {
  voices = synth
    ...
    .filter((voice) => {
      ...
      return voiceLanguageCode === getLanguageCode(LANGUAGE);
    })
    .sort(function (a, b) {
...
function speak(text = "Ciao, come stai?") {
  if (synth.speaking) {
  ...
  utterThis.onboundary = (event) => {
    ...
    const calc =
      totalLength - (event.charIndex / totalTextLength) * totalLength;
...
btn.addEventListener("click", () =>
  synth?.speaking ? pauseAudio() : playAudio()
);
